import React from "react";
export const Experience = () => {
  return (
    <div>
      <article
        className="xl:flex xl:flex-row gap-10 mb-16 mobiles:flex-col mobiles:gap-6"
        id="stack"
      >
        <div>
          <h5 className="text-sm text-nowrap">Ene 2022 - Actualidad</h5>
        </div>
        <div className="">
          <h2 className="text-l font-semibold text-white mb-2">
            Desarrollador Freelance ·{" "}
            <a
              href="https://www.bixo.pro"
              target="_blank"
              className="underline text-stack font-semibold"
            >
              Bixo
            </a>
          </h2>
          <p className="text-sm line leading-6 max-w-80">
            Desarrollador Freelance, actualmente para mi agencia Bixo. Creación
            e implementación de Apis rest, diseño y puesta en práctica de
            interfaces de usuario, optimización, seguridad, SEO, Hosting,
            Responsive.
          </p>
          <aside className="flex gap-2 mt-4 flex-wrap">
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              React
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Redux
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              HTML5
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              {" "}
              Tailwind
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              SCSS
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Node
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Express
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Django
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              SQL
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Mongo
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Git
            </h4>
          </aside>
        </div>
      </article>
      <article
        className="xl:flex xl:flex-row gap-10 mb-16 mobiles:flex-col mobiles:gap-6"
        id="stack"
      >
        <div>
          <h5 className="text-sm text-nowrap">Nov 2023 - Mar 2024</h5>
        </div>
        <div className="">
          <h2 className="text-l font-semibold text-white mb-2">
            Desarrollador React Native · PalCare
          </h2>
          <p className="text-sm line leading-6 max-w-80">
            Desarrollador para un proyecto mobile y web donde se debía
            desarrollar una aplicación capaz de vincularse a una serie de
            modelos de pulseras Bluetooth. Me relacioné con varios equipos de
            trabajo, tanto diseñadores como desarrolladores con el fin de
            brindar la mejor experiencia de usuario
          </p>
          <aside className="flex gap-2 mt-4">
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Javascript
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Node
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl text-nowrap">
              {" "}
              React Native
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Typescript
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Git
            </h4>
          </aside>
        </div>
      </article>
      <article
        className="xl:flex xl:flex-row gap-10 mb-16 mobiles:flex-col mobiles:gap-6"
        id="stack"
      >
        <div>
          <h5 className="text-nowrap text-sm">Sep 2023 - Ene 2024</h5>
        </div>
        <div className="">
          <h2 className="text-l font-semibold text-white mb-2">
            Desarrollador Fullstack · Smart Profit Investments
          </h2>
          <p className="text-sm line leading-6 max-w-80">
            Desarrollador Fullstack en relación de dependencia para una Empresa
            Costarricense. Encargado de diseñar y desarrollar los sitios para
            cada una de las ramificaciones de la empresa. Responsable de
            Estructurar y desarrollar el Backend e integrarlo en una interfaz de
            estetica y optimizada.
          </p>
          <aside className="flex flex-wrap gap-2 mt-4">
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Node
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              {" "}
              React
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Redux
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Typescript
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              HTML5
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              SCSS
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Tailwind
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Python
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Django
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              SQL
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              AWS
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              DRF
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Git
            </h4>
          </aside>
        </div>
      </article>
      <article
        className="xl:flex xl:flex-row gap-10 mb-16 mobiles:flex-col mobiles:gap-6"
        id="stack"
      >
        <div>
          <h5 className="text-nowrap text-sm">Mar 2022 - May 2023</h5>
        </div>
        <div className="">
          <h2 className="text-l font-semibold text-white mb-2">
            Desarrollador Backend · Pipeline.io
          </h2>
          <p className="text-sm line leading-6 max-w-80">
            Desarrollador backend para una consultora Europea como Freelancer.
            Desarrollo de Api rest, pruebas unitarias, Documentación y
            despliegue a producción.
          </p>
          <aside className="flex flex-wrap gap-2 mt-4">
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Python
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Django
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Django DRF
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              SQL
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Mongo
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Docker
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Graphql
            </h4>
            <h4 className="text-sm bg-stackback text-stack p-2 rounded-xl">
              Git
            </h4>
          </aside>
        </div>
      </article>
    </div>
  );
};
